...
class OTPSubmitModal(discord.ui.Modal, title="Embassy OTP Verification"):
    otp = discord.ui.TextInput(
        label="Company OTP",
        placeholder="Enter the exact OTP company name",
        min_length=1,
        max_length=100,
        required=True,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        if complete_cog is None:
            ...
            return
        try:
            verified, attempts, lock_until = await complete_cog.verification.verify_company_otp(
                self.request_id, self.otp.value, interaction.user.id
            )
        ...
        if verified:
            await self.service.database.collection("requests").update_one(
                {"request_id": self.request_id}, {"$set": {"status": "VERIFIED", "active": True}}
            )
            ...
            await interaction.channel.send(
                embed=discord.Embed(
                    title="✅ WarEra Verification Complete",
                    description="Your WarEra identity and company ownership have been verified.\n\nYour request can now proceed to embassy access review.",
    ...
    async def continue_verification(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"**Official status:** {official_text}\n\n"
                f"**Your OTP:** `{otp}`\n\n"
...
class EmbassyRequestsCog(commands.Cog):
    ...
    async def cog_load(self) -> None:
        ...
        self.bot.add_view(OTPSubmitView(self.service, "persistent-placeholder"))
