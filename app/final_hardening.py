...
async def _safe_decide_workflow(self: ApprovalWorkflow, request_id: str, actor_id: int, decision: Decision, route: Route, reason: str | None = None) -> bool:
    current = await self.requests.find_one({"request_id": request_id, "active": True, "state": {"$in": list(_REVIEW_STATES)}})
    if not current:
        return False
    ...
    result = await self.requests.update_one(
        ...
        {"$set": {"state": new_state, "decision": decision.value, "decision_actor_id": actor_id, "decision_reason": reason, "updated_at": now, "active": False}},
    )
...
async def _rollback_decision(self: ApprovalWorkflow, request_id: str, actor_id: int) -> None:
    ...
    await self.requests.update_one(
        ...
        {"$set": {"state": previous_state, "active": True, "decision": None, "decision_actor_id": None, "decision_reason": None, "updated_at": datetime.now(timezone.utc)}},
    )
...
async def _safe_embassy_decide(self: EmbassyFlow, interaction: discord.Interaction, request_id: str, decision: Decision, route: Route) -> None:
    ...
    if route is Route.FOREIGN_DIPLOMAT:
        ...
        if not isinstance(interaction.user, discord.Member) or not (interaction.user.guild_permissions.administrator or any(role.id == settings.role_eam_id for role in interaction.user.roles)):
    ...
    if decision is Decision.APPROVED:
        try:
            await self._grant_access(interaction.guild, applicant_id, embassy, AccessSource.DIPLOMAT_APPROVAL if route is Route.FOREIGN_DIPLOMAT else AccessSource.GOVERNMENT_OVERRIDE)
        except Exception:
            await workflow.rollback_decision(request_id, interaction.user.id)
            await self.audit.log(action="ACCESS_GRANT_FAILED_AFTER_APPROVAL_CLAIM", actor_id=interaction.user.id, request_id=request_id, embassy_id=embassy.embassy_id)
        ...
        await _dm_applicant(self, interaction.guild, applicant_id, approved=False, embassy_name=embassy.country_name)
