...
class FixedVerificationDashboardView(discord.ui.View):
    ...
    async def start_request(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        ...
        try:
            ...
            logger.exception("Could not create verification request for user %s", interaction.user.id)
            if request is not None:
                try:
                    await store.cancel_request(str(request["id"]), reason="Discord request setup failed")
                except Exception:
                    logger.exception("Could not cancel orphaned request %s", request.get("id"))
            if thread is not None:
                try:
                    await thread.delete(reason="RAJDOOT verification request setup failed")
                except (discord.Forbidden, discord.NotFound, discord.HTTPException):
                    logger.exception("Could not delete orphaned request thread %s", thread.id)
            await progress.edit(content="⚠️ RAJDOOT could not create the private request. No usable request was left behind; please try again.")
