...
class FixedGovernmentDashboardView(discord.ui.View):
    ...
    async def diplomats(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        await self._open(interaction, embed=discord.Embed(title="👥 Manage Diplomats", description="Diplomat profiles, assignments and access management will open here.", colour=discord.Colour.blurple()))
    ...
    async def statistics(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        await self._open(interaction, embed=discord.Embed(title="📊 Government Statistics", description="Government and embassy statistics will open here.", colour=discord.Colour.blurple()))
    ...
    async def logs(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        await self._open(interaction, embed=discord.Embed(title="📜 RAJDOOT Logs", description="Operational history and audit information will open here.", colour=discord.Colour.blurple()))
...
class FixedDiplomatDashboardView(discord.ui.View):
    ...
    async def information(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        await self._open(interaction, embed=discord.Embed(title="📋 Embassy Information", description="Embassy information, contacts and diplomatic records will be shown here.", colour=discord.Colour.blurple()))
    ...
    async def preapproval(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        await self._open(interaction, embed=discord.Embed(title="🤝 Pre-Approve Visitor", description="Visitor pre-approval will be handled entirely through this dashboard workflow.", colour=discord.Colour.green()))
