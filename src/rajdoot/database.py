...
class Database:
    ...
    async def fetch_pending_requests_for_member(self, discord_user_id: int) -> list[dict[str, Any]]:
        ...
        assert self._connection is not None
        async with self._connection.cursor() as cursor:
            await cursor.execute("select r.*, e.country_name, e.channel_id, e.channel_name from embassy_requests r join embassies e on e.id = r.embassy_id where r.request_status = 'pending_approval' order by r.created_at asc")
            return list(await cursor.fetchall())
