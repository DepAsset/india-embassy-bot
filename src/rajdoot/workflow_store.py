...
class WorkflowStore:
    ...
    async def set_flow_state(self, request_id: str, stage: str, **fields: Any) -> None:
        ...
        allowed = {
            "warera_user_id", "warera_profile_snapshot", "government_position",
            ...
            "target_embassy_id", "preapproval_id", "verification_status", "request_status",
            "verification_started_at", "verification_completed_at", "last_verification_error",
        }
    ...
    async def cancel_request(self, request_id: str, *, reason: str) -> bool:
        """Close an unfinished request when its Discord thread was already closed."""
        connection = await self._connection()
        async with connection.transaction():
            async with connection.cursor() as cursor:
                await cursor.execute(
    ...
    async def fetch_request(self, request_id: str) -> dict[str, Any] | None:
        ...
        async with connection.cursor() as cursor:
            await cursor.execute(
                "select * from embassy_requests where id = %s::uuid",
                (request_id,),
            )
    ...
    async def record_company_attempt(self, request_id: str, company_match: bool) -> tuple[str, dict[str, Any] | None]:
        ...
        async with connection.transaction():
            async with connection.cursor() as cursor:
                await cursor.execute("set local lock_timeout = '3s'")
                ...
                maximum = int(row.get("verification_max_attempts") or 5)
                if company_match:
                    await cursor.execute(
                        """
                        update embassy_requests
                    ...
                    return "verified", dict(result) if result else None
                if attempts >= maximum:
                    await cursor.execute(
                        """
                        update embassy_requests
                    ...
                    result = await cursor.fetchone()
                    return "max_attempts", dict(result) if result else None
                await cursor.execute(
                    """
                    update embassy_requests
                    set verification_attempts = %s,
                        last_verification_error = 'Company OTP not found',
                        updated_at = now()
                    where id = %s::uuid
                    returning *
                    """,
                ...
                result = await cursor.fetchone()
                return "invalid", dict(result) if result else None
    ...
    async def active_assignments_for_user(self, discord_user_id: int) -> list[dict[str, Any]]:
        ...
        async with connection.cursor() as cursor:
            await cursor.execute(
                ...
                from embassy_assignments a
                join embassies e on e.id = a.embassy_id
                where a.user_discord_id = %s
                  and a.status = 'active'
                  and e.status = 'active'
        ...
        async with connection.transaction():
            async with connection.cursor() as cursor:
                ...
                await cursor.execute(
                    ...
                    (embassy_id, discord_user_id, discord_username, member_type, active)
                    values (%s::uuid, %s, %s, %s, true)
                    on conflict (embassy_id, discord_user_id)
                    do update set member_type = excluded.member_type, active = true, updated_at = now()
                    """,
                    (embassy_id, str(user_discord_id), str(user_discord_id), assignment_type),
                )
                return dict(row)
                ...
                if changed:
                    await cursor.execute(
                        ...
                        where embassy_id = %s::uuid
                          ...
                          and not exists (
                              select 1 from embassy_assignments
                              where embassy_id = %s::uuid
                                and user_discord_id = %s
                                and status = 'active'
                          )
                        """,
                        (embassy_id, str(user_discord_id), embassy_id, user_discord_id),
                    )
                return changed
    ...
    async def log_audit(
        ...
        *,
        ...
        async with connection.cursor() as cursor:
            await cursor.execute(
                "insert into audit_logs (actor_discord_id, action, target_type, target_id, embassy_id, result, metadata) "
