...
class WorkflowStore:
    """Fast SQL primitives for the embassy workflow.

    The existing Database class remains the home of the legacy registry methods.
    This store keeps the new workflow queries together so each interaction can
    use one short transaction instead of several sequential round trips.
    """
    ...
    async def set_flow_state(self, request_id: str, stage: str, **fields: Any) -> None:
        connection = await self._cursor()
        allowed = {
            "flow_stage", "warera_user_id", "warera_profile_snapshot", "government_position",
            "government_country_id", "government_auto_approved", "request_thread_id",
            "approval_message_id", "request_log_message_id", "target_country_id", "target_embassy_id",
            "preapproval_id", "verification_status", "request_status", "verification_started_at",
            "verification_completed_at", "last_verification_error",
        }
        ...
        for key, value in fields.items():
            ...
            if key in {"warera_profile_snapshot"}:
        ...
        async with connection.transaction():
            async with connection.cursor() as cursor:
                await cursor.execute(
                    ...
                    values (%s, 'pending', 'created', 'profile_pending')
                    returning *
                    """,
                    (applicant_id,),
                )
    ...
    async def issue_otp(self, request_id: str, otp_hash: str, otp: str) -> dict[str, Any] | None:
        # OTP itself is never persisted; only its SHA-256 hash is stored.
        connection = await self._cursor()
        async with connection.transaction():
            async with connection.cursor() as cursor:
                await cursor.execute(
                    ...
                    set otp_hash = %s,
                        ...
                        flow_stage = 'company_verification',
                        updated_at = now()
                    where id = %s::uuid
                    returning *
                    """,
    ...
    async def verify_otp_attempt(self, request_id: str, otp_hash: str) -> tuple[str, dict[str, Any] | None]:
        connection = await self._cursor()
        async with connection.transaction():
            async with connection.cursor() as cursor:
                await cursor.execute(
                    "select * from embassy_requests where id = %s::uuid for update",
                    (request_id,),
                )
                row = await cursor.fetchone()
                if row is None:
                ...
                if int(row.get("verification_attempts") or 0) >= int(row.get("verification_max_attempts") or 5):
                    return "max_attempts", dict(row)
                if row.get("otp_hash") != otp_hash:
                    await cursor.execute(
                        """
                        update embassy_requests
                    ...
                    if failed and int(failed["verification_attempts"]) >= int(failed["verification_max_attempts"]):
                        await cursor.execute(
                            """
                            update embassy_requests
                            set verification_status = 'failed', request_status = 'failed',
                                flow_stage = 'verification_failed', last_verification_error = 'Maximum company verification attempts reached',
                                updated_at = now()
                            where id = %s::uuid
                            returning *
                            """,
                    ...
                    return "invalid", dict(failed) if failed else None
                await cursor.execute(
                    """
                    update embassy_requests
                    set verification_status = 'verified', request_status = 'created',
    ...
    async def create_preapproval(
        ...
        *,
        embassy_id: str,
        diplomat_discord_id: int,
        visitor_warera_id: str,
        visitor_profile_url: str | None,
